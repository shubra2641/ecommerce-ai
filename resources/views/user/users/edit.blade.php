...
@section('title', getPageTitle(__('user.edit_user')))

@section('main-content')

<div class="card">
  <h5 class="card-header">{{ __('user.edit_user') }}</h5>
    <div class="card-body">
      <form method="post" action="{{route('users.update',$user->id)}}">
        ...
        <input id="inputTitle" type="text" name="name" placeholder="{{ __('user.form.enter_name') }}"  value="{{$user->name}}" class="form-control">
        @error('name')
        ...
        <div class="form-group">
          ...
          <input id="inputEmail" type="email" name="email" placeholder="{{ __('user.form.enter_email') }}"  value="{{$user->email}}" class="form-control">
          @error('email')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
  ...
  <img id="holder" class="mt-3 user-photo-preview">
        ...
        <div class="form-group">
            ...
            <select name="role" class="form-control">
                ...
                @foreach($roles as $role)
                    <option value="{{$role->role}}" {{(($role->role=='admin') ? 'selected' : '')}}>{{ __('user.admin') }}</option>
                    <option value="{{$role->role}}" {{(($role->role=='user') ? 'selected' : '')}}>{{ __('user.user') }}</option>
                @endforeach
            </select>
          @error('role')
          <span class="text-danger">{{$message}}</span>
        ...
        <div class="form-group mb-3">
           <button class="btn btn-success" type="submit">{{ __('user.form.update') }}</button>
        </div>
      </form>
    </div>
</div>
