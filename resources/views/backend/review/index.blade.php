...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary float-left">{{ trans('app.all_reviews') }}</h6>
    </div>
    <div class="card-body">
      <div class="table-responsive">
          ...
          <tbody>
            @foreach($reviews as $review)
                <tr>
                    ...
                    <td>
                        <a href="{{route('review.edit',$review->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="{{ trans('app.edit') }}" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('review.destroy',[$review->id])}}">
                          @csrf
                          @method('delete')
                              <button class="btn btn-danger btn-sm dltBtn" data-id={{$review->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="{{ trans('app.delete') }}"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </td>
                </tr>
            @endforeach
        ...
        @else
          <h6 class="text-center">{{ trans('app.no_data_found') }}!!!</h6>
        @endif
      </div>
    </div>
</div>
