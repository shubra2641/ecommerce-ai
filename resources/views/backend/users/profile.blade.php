...
@section('title', trans('app.user_profile'))
...
<div class="card shadow mb-4">
   ...
   <div class="card-header py-3">
     <h4 class=" font-weight-bold">{{ trans('app.profile') }}</h4>
     <ul class="breadcrumbs">
         <li><a href="{{route('admin')}}" style="color:#999">{{ trans('app.dashboard') }}</a></li>
         <li><a href="" class="active text-primary">{{ trans('app.profile') }}</a></li>
     </ul>
   </div>
   <div class="card-body">
        <div class="row">
            ...
            <div class="col-md-8">
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('profile-update',$profile->id)}}">
                    ...
                    <div class="form-group">
                      ...
                      <input id="inputTitle" type="text" name="name" placeholder="{{ trans('app.name') }}"  value="{{$profile->name}}" class="form-control">
                      @error('name')
                      ...
                      <div class="form-group">
                        ...
                        <input id="inputEmail" disabled type="email" name="email" placeholder="{{ trans('app.email') }}"  value="{{$profile->email}}" class="form-control">
                        @error('email')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                      ...
                      <div class="form-group">
                          ...
                          <select name="role" class="form-control">
                              <option value="">-----{{ trans('app.role') }}-----</option>
                                  <option value="admin" {{(($profile->role=='admin')? 'selected' : '')}}>{{ trans('app.admin') }}</option>
                                  <option value="user" {{(($profile->role=='user')? 'selected' : '')}}>{{ trans('app.user') }}</option>
                          </select>
                        @error('role')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
