...
class OrderController extends Controller
{
    ...
    public function store(OrderStoreRequest $request): RedirectResponse
    {
        try {
            ...
            if ($order) {
                ...
                if (in_array($paymentMethod, ['paypal', 'stripe', 'tap'])) {
                    ...
                    // For offline payments (COD, bank transfer), process directly
                    $gateway = PaymentGateway::where('slug', $paymentMethod)->first();
                    if ($gateway) {
                        // Handle offline payment processing
    ...
    private function validateOrderRequest(Request $request): array
    {
        $rules = [
            ...
            'payment_method' => 'required|string|in:paypal,stripe,tap,cod'
        ];

        // Add payment gateway validation if needed
        $selectedMethod = $request->input('payment_method');
        if ($selectedMethod && !in_array($selectedMethod, ['paypal', 'stripe', 'tap', 'cod'])) {
    ...
    private function prepareOrderData(Request $request, array $validatedData): array
    {
        $orderData = $request->only([
            ...
            'phone', 'post_code', 'email', 'country', 'shipping'
        ]);

        $orderData['order_number'] = 'ORD-' . strtoupper(Str::random(10));
        $orderData['user_id'] = auth()->id();
        $orderData['shipping_id'] = $request->input('shipping');
        ...
        $orderData['total_amount'] = $this->calculateTotalAmount($request);

        // Handle payment method
        $this->setPaymentDetails($orderData, $request);
     ...
     * @return float
     */
    private function calculateTotalAmount(Request $request): float
    {
     ...
     * @return void
     */
    private function setPaymentDetails(array &$orderData, Request $request): void
    {
        $paymentMethod = $request->input('payment_method');
    ...
    /**
     * Update the specified order (duplicate method - should be removed)
     * 
     * @param \App\Http\Requests\Frontend\OrderUpdateRequest $request
     * @param int $id
     * @return RedirectResponse
     */
    public function updateDuplicate(\App\Http\Requests\Frontend\OrderUpdateRequest $request, int $id): RedirectResponse
    {
        try {
            $order = Order::findOrFail($id);
     ...
     * 
     * @param \App\Http\Requests\Frontend\OrderTrackRequest $request
     * @return View|RedirectResponse
     */
    public function productTrackOrder(\App\Http\Requests\Frontend\OrderTrackRequest $request)
    {
        try {
            $validatedData = $request->validated();
     ...
     * 
     * @param \App\Http\Requests\Frontend\OrderPdfRequest $request
     * @return \Illuminate\Http\Response
     */
    public function pdf(\App\Http\Requests\Frontend\OrderPdfRequest $request)
    {
        try {
            $validatedData = $request->validated();
